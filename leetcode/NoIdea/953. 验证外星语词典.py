...
class Solution:
    ...
    def isAlienSorted(self, words, order):
        ...
        for index, value in enumerate(order):
            order_dict[value] = index+1

        last_word = words[0]
        for i in range(1, len(words)):
            flag = 0
            for x, y in zip(last_word, words[i]):
                if order_dict[x] < order_dict[y]:
                    flag = 1
                    break
                elif order_dict[x] > order_dict[y]:
                    return False
            # 如果两个单词前面一摸一样，那么比较长度，短的应该在拍在前面
            if flag == 0 and len(last_word) > len(words[i]):
                return False
            last_word = words[i]
        return True
